...
//include SITE_PATH.'admin/mail/mimePart.php';
App::uses('Mail_mimePart','Lib');
class htmlMimeMail
{
	/**
...
*/
	function &_addTextPart(&$obj, $text)
	{
		if (isset($this->headers['Content-Type'])) {
			$params['content_type'] = $this->headers['Content-Type'];
			$params['encoding']     = $this->headers['Content-Transfer-Encoding'];
		} else {
...
*/
	...
	{
		$params['content_type'] = isset($this->headers['Content-Type']) ? $this->headers['Content-Type'] : 'text/html';
		$params['encoding']     = isset($this->headers['Content-Transfer-Encoding']) ? $this->headers['Content-Transfer-Encoding'] : $this->build_params['html_encoding'];
...
*/
	...
	{
		$params['content_type'] = isset($this->headers['Content-Type']) ? $this->headers['Content-Type'] : 'multipart/mixed';
...
*/
	...
	{
		$params['content_type'] = isset($this->headers['Content-Type']) ? $this->headers['Content-Type'] : 'multipart/alternative';
		if (is_object($obj)) {
...
*/
	...
	{
		$params['content_type'] = isset($this->headers['Content-Type']) ? $this->headers['Content-Type'] : 'multipart/related';
		if (is_object($obj)) {
			return $obj->addSubpart('', $params);
		} else {
			return new Mail_mimePart('', $params);
...
//echo "<DIV style='{COLOR : black; FONT-SIZE : 12px; BACKGROUND-COLOR : #cccccc;}'><PRE>";
			...
			case 'mail':
				...
				foreach ($this->headers as $name => $value) {
					if ($name) {
						...
						$headers[] = $this->_encodeHeader($value, $this->build_params['head_charset']);
					}
				}
...
*/
	...
	{
		$key = implode("", file(FILE_KEY));
		
		$fp = fopen(TPL_PATH."tmp/msg.txt","w");
		fclose($fp);
		$fp = fopen(TPL_PATH."tmp/enc.txt","w");
		fclose($fp);
		...
		$res = openssl_pkcs7_encrypt(TPL_PATH."tmp/msg.txt", TPL_PATH."tmp/enc.txt", $key, $this->headers);
		
		if (!$res) {
			$res = "";
			while ($msg = openssl_error_string()) $res.=$msg;
			return $res;
		}
		...
		$fp = fopen(TPL_PATH."tmp/enc.txt", "r");
		while (!feof($fp)) {
      ...
      //if ($buffer=="") $message_flag = 1;
			
			if (trim($buffer)=="")
				if (!$message_flag && $message == "") $message_flag = 1; else $message_flag = 0;
			//echo htmlspecialchars($buffer)."---".strlen($buffer)."---$message_flag---<br>";
			
			if ($message_flag) {
				if (trim($buffer)!="") $message .= $buffer;
			} else {
				$headers = explode(':', $buffer);
				if ($headers[0] && $headers[1]) $this->setHeader(trim($headers[0]), trim($headers[1]));
			}
		}
		fclose($fp);
//echo "<DIV style='{COLOR : black; FONT-SIZE : 12px; BACKGROUND-COLOR : #cccccc;}'><PRE>";
...
//echo "</PRE></DIV>";
		...
		$this->text = '';
		//echo htmlspecialchars($this->text).'---<br>';
